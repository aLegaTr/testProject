...
public class UserService implements UserDetailsService {
    ...
    private void sentMessage(User user) {
        if (!StringUtils.isEmpty(user.getEmail())){
            String message=String.format(
                    "Hello, %s! \n " +
                    "Welcome !!! Please visit next link:http://localhost:8080/activate/%s", user.getUsername(), user.getActivationCode());
    ...
    public void subcribe(User currentUser, User user) {
        user.getSubscribers().add(currentUser);
        userRepo.save(user);
    }

    public void unSubcribe(User currentUser, User user) {
